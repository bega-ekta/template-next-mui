import '@mui/lab/themeAugmentation';
import { alpha, createTheme, darken } from '@mui/material';

import { darkColors, darkThemeColors } from './darkColors';

export const darkTheme = createTheme({
  // direction: i18n.dir(),
  colors: {
    gradients: {
      blue1: darkColors.gradients.blue1,
      blue2: darkColors.gradients.blue2,
      blue3: darkColors.gradients.blue3,
      blue4: darkColors.gradients.blue4,
      blue5: darkColors.gradients.blue5,
      orange1: darkColors.gradients.orange1,
      orange2: darkColors.gradients.orange2,
      orange3: darkColors.gradients.orange3,
      purple1: darkColors.gradients.purple1,
      purple3: darkColors.gradients.purple3,
      pink1: darkColors.gradients.pink1,
      pink2: darkColors.gradients.pink2,
      green1: darkColors.gradients.green1,
      green2: darkColors.gradients.green2,
      black1: darkColors.gradients.black1,
      black2: darkColors.gradients.black2,
    },
    shadows: {
      success: darkColors.shadows.success,
      error: darkColors.shadows.error,
      primary: darkColors.shadows.primary,
      info: darkColors.shadows.info,
      warning: darkColors.shadows.warning,
    },
    alpha: {
      white: {
        5: alpha(darkThemeColors.white, 0.02),
        10: alpha(darkThemeColors.white, 0.1),
        30: alpha(darkThemeColors.white, 0.3),
        50: alpha(darkThemeColors.white, 0.5),
        70: alpha(darkThemeColors.white, 0.7),
        100: darkThemeColors.white,
      },
      trueWhite: {
        5: alpha(darkThemeColors.trueWhite, 0.02),
        10: alpha(darkThemeColors.trueWhite, 0.1),
        30: alpha(darkThemeColors.trueWhite, 0.3),
        50: alpha(darkThemeColors.trueWhite, 0.5),
        70: alpha(darkThemeColors.trueWhite, 0.7),
        100: darkThemeColors.trueWhite,
      },
      black: {
        5: alpha(darkThemeColors.black, 0.02),
        10: alpha(darkThemeColors.black, 0.1),
        30: alpha(darkThemeColors.black, 0.3),
        50: alpha(darkThemeColors.black, 0.5),
        70: alpha(darkThemeColors.black, 0.7),
        100: darkThemeColors.black,
      },
    },
    secondary: {
      lighter: alpha(darkThemeColors.secondary, 0.1),
      light: alpha(darkThemeColors.secondary, 0.3),
      main: darkThemeColors.secondary,
      dark: darken(darkThemeColors.secondary, 0.2),
    },
    primary: {
      lighter: alpha(darkThemeColors.primary, 0.1),
      light: alpha(darkThemeColors.primary, 0.3),
      main: darkThemeColors.primary,
      dark: darken(darkThemeColors.primary, 0.2),
    },
    success: {
      lighter: alpha(darkThemeColors.success, 0.1),
      light: alpha(darkThemeColors.success, 0.3),
      main: darkThemeColors.success,
      dark: darken(darkThemeColors.success, 0.2),
    },
    warning: {
      lighter: alpha(darkThemeColors.warning, 0.1),
      light: alpha(darkThemeColors.warning, 0.3),
      main: darkThemeColors.warning,
      dark: darken(darkThemeColors.warning, 0.2),
    },
    error: {
      lighter: alpha(darkThemeColors.error, 0.1),
      light: alpha(darkThemeColors.error, 0.3),
      main: darkThemeColors.error,
      dark: darken(darkThemeColors.error, 0.2),
    },
    info: {
      lighter: alpha(darkThemeColors.info, 0.1),
      light: alpha(darkThemeColors.info, 0.3),
      main: darkThemeColors.info,
      dark: darken(darkThemeColors.info, 0.2),
    },
  },
  general: {
    reactFrameworkColor: '#00D8FF',
    borderRadiusSm: '6px',
    borderRadius: '10px',
    borderRadiusLg: '12px',
    borderRadiusXl: '16px',
  },
  sidebar: {
    background: darkColors.layout.sidebar.background,
    textColor: darkColors.layout.sidebar.textColor,
    dividerBg: darkColors.layout.sidebar.dividerBg,
    menuItemColor: darkColors.layout.sidebar.menuItemColor,
    menuItemColorActive: darkColors.layout.sidebar.menuItemColorActive,
    menuItemBg: darkColors.layout.sidebar.menuItemBg,
    menuItemBgActive: darkColors.layout.sidebar.menuItemBgActive,
    menuItemIconColor: darkColors.layout.sidebar.menuItemIconColor,
    menuItemIconColorActive: darkColors.layout.sidebar.menuItemIconColorActive,
    menuItemHeadingColor: darkColors.layout.sidebar.menuItemHeadingColor,
    boxShadow: '1px 0 0 #272C48',
    width: '290px',
  },
  header: {
    height: '80px',
    background: darkThemeColors.primaryAlt,
    boxShadow: '0px 1px 0px #272C48',
    textColor: darkColors.secondary.main,
  },
  spacing: 9,
  palette: {
    common: {
      black: darkColors.alpha.black[100],
      white: darkColors.alpha.white[100],
    },
    mode: 'dark',
    primary: {
      light: darkColors.primary.light,
      main: darkColors.primary.main,
      dark: darkColors.primary.dark,
    },
    secondary: {
      light: darkColors.secondary.light,
      main: darkColors.secondary.main,
      dark: darkColors.secondary.dark,
    },
    error: {
      light: darkColors.error.light,
      main: darkColors.error.main,
      dark: darkColors.error.dark,
      contrastText: darkThemeColors.trueWhite,
    },
    success: {
      light: darkColors.success.light,
      main: darkColors.success.main,
      dark: darkColors.success.dark,
      contrastText: darkThemeColors.trueWhite,
    },
    info: {
      light: darkColors.info.light,
      main: darkColors.info.main,
      dark: darkColors.info.dark,
      contrastText: darkThemeColors.trueWhite,
    },
    warning: {
      light: darkColors.warning.light,
      main: darkColors.warning.main,
      dark: darkColors.warning.dark,
      contrastText: darkThemeColors.trueWhite,
    },
    text: {
      primary: darkColors.alpha.black[100],
      secondary: darkColors.alpha.black[70],
      disabled: darkColors.alpha.black[50],
    },
    background: {
      paper: darkColors.alpha.white[100],
      default: darkColors.layout.general.bodyBg,
    },
    action: {
      active: darkColors.alpha.black[100],
      hover: darkColors.primary.lighter,
      hoverOpacity: 0.1,
      selected: darkColors.alpha.black[10],
      selectedOpacity: 0.1,
      disabled: darkColors.alpha.black[50],
      disabledBackground: darkColors.alpha.black[5],
      disabledOpacity: 0.38,
      focus: darkColors.alpha.black[10],
      focusOpacity: 0.05,
      activatedOpacity: 0.12,
    },
  },
  breakpoints: {
    values: {
      xs: 0,
      sm: 600,
      md: 960,
      lg: 1280,
      xl: 1840,
    },
  },
  components: {
    MuiBackdrop: {
      styleOverrides: {
        root: {
          backgroundColor: alpha(darken('#070C27', 0.5), 0.4),
          backdropFilter: 'blur(2px)',

          '&.MuiBackdrop-invisible': {
            backgroundColor: 'transparent',
            backdropFilter: 'blur(2px)',
          },
        },
      },
    },
    MuiFormHelperText: {
      styleOverrides: {
        root: {
          textTransform: 'none',
          marginLeft: 8,
          marginRight: 8,
          fontWeight: 'bold',
        },
      },
    },
    MuiDialog: {
      styleOverrides: {
        paper: {
          backgroundColor: darken(darkThemeColors.primaryAlt, 0.5),
        },
      },
    },
    MuiCssBaseline: {
      styleOverrides: {
        'html, body': {
          width: '100%',
          height: '100%',
        },
        body: {
          display: 'flex',
          flexDirection: 'column',
          minHeight: '100%',
          width: '100%',
          flex: 1,
        },
        '#__next': {
          width: '100%',
          display: 'flex',
          flex: 1,
          flexDirection: 'column',
        },
        html: {
          display: 'flex',
          flexDirection: 'column',
          minHeight: '100%',
          width: '100%',
          MozOsxFontSmoothing: 'grayscale',
          WebkitFontSmoothing: 'antialiased',
        },
        a: {
          textDecoration: 'none',
          color: 'inherit',
        },
        ul: {
          margin: 0,
          padding: 0,
          listStyle: 'none',
        },
        li: {
          listStyle: 'none',
        },
        '.child-popover .MuiPaper-root .MuiList-root': {
          flexDirection: 'column',
        },
        '#nprogress': {
          pointerEvents: 'none',
        },
        '#nprogress .bar': {
          background: darkColors.primary.lighter,
        },
        '#nprogress .spinner-icon': {
          borderTopColor: darkColors.primary.lighter,
          borderLeftColor: darkColors.primary.lighter,
        },
        '#nprogress .peg': {
          boxShadow: '0 0 15px ' + darkColors.primary.lighter + ', 0 0 8px' + darkColors.primary.light,
        },
        ':root': {
          '--swiper-theme-color': darkColors.primary.main,
          colorScheme: 'dark',
        },
        code: {
          background: darkColors.info.lighter,
          color: darkColors.alpha.black[100],
          borderRadius: 4,
          padding: 4,
        },
        '@keyframes pulse': {
          '0%': {
            transform: 'scale(.75)',
          },
          '20%': {
            transform: 'scale(1.1)',
          },
          '40%': {
            transform: 'scale(.75)',
          },
          '60%': {
            transform: 'scale(1.05)',
          },
          '80%': {
            transform: 'scale(.75)',
          },
          '100%': {
            transform: 'scale(.75)',
          },
        },
        '@keyframes ripple': {
          '0%': {
            transform: 'scale(.8)',
            opacity: 1,
          },
          '100%': {
            transform: 'scale(2.8)',
            opacity: 0,
          },
        },
        '@keyframes float': {
          '0%': {
            transform: 'translate(0%, 0%)',
          },
          '100%': {
            transform: 'translate(3%, 3%)',
          },
        },
      },
    },
    MuiSelect: {
      styleOverrides: {
        iconOutlined: {
          color: darkColors.alpha.black[50],
        },
        icon: {
          top: 'calc(50% - 14px)',
        },
      },
    },
    MuiOutlinedInput: {
      styleOverrides: {
        root: {
          '& .MuiInputAdornment-positionEnd.MuiInputAdornment-outlined': {
            paddingRight: 6,
          },
          '&:hover .MuiOutlinedInput-notchedOutline': {
            borderColor: darkColors.alpha.black[50],
          },
          '&.Mui-focused:hover .MuiOutlinedInput-notchedOutline': {
            borderColor: darkColors.primary.main,
          },
        },
      },
    },
    MuiListSubheader: {
      styleOverrides: {
        colorPrimary: {
          fontWeight: 'bold',
          lineHeight: '40px',
          fontSize: 13,
          background: darkColors.alpha.black[5],
          color: darkColors.alpha.black[70],
        },
      },
    },
    MuiCardHeader: {
      styleOverrides: {
        action: {
          marginTop: -5,
          marginBottom: -5,
        },
        title: {
          fontSize: 15,
        },
      },
    },
    MuiRadio: {
      styleOverrides: {
        root: {
          borderRadius: '50px',
        },
      },
    },
    MuiChip: {
      styleOverrides: {
        colorSecondary: {
          background: darkColors.alpha.black[5],
          color: darkColors.alpha.black[100],

          '&:hover': {
            background: darkColors.alpha.black[10],
          },
        },
        deleteIcon: {
          color: darkColors.alpha.black[50],

          '&:hover': {
            color: darkColors.alpha.black[70],
          },
        },
      },
    },
    MuiAccordion: {
      styleOverrides: {
        root: {
          boxShadow: 'none',

          '&.Mui-expanded': {
            margin: 0,
          },
          '&::before': {
            display: 'none',
          },
        },
      },
    },
    MuiAvatar: {
      styleOverrides: {
        root: {
          fontSize: 14,
          fontWeight: 'bold',
        },
        colorDefault: {
          background: darkColors.alpha.black[30],
          color: darkColors.alpha.trueWhite[100],
        },
      },
    },
    MuiAvatarGroup: {
      styleOverrides: {
        root: {
          alignItems: 'center',
        },
        avatar: {
          background: darkColors.alpha.black[10],
          fontSize: 13,
          color: darkColors.alpha.black[70],
          fontWeight: 'bold',

          '&:first-of-type': {
            border: 0,
            background: 'transparent',
          },
        },
      },
    },
    MuiListItemAvatar: {
      styleOverrides: {
        alignItemsFlexStart: {
          marginTop: 0,
        },
      },
    },
    MuiPaginationItem: {
      styleOverrides: {
        page: {
          fontSize: 13,
          fontWeight: 'bold',
          transition: 'all .2s',
        },
        textPrimary: {
          '&.Mui-selected': {
            boxShadow: darkColors.shadows.primary,
          },
          '&.MuiButtonBase-root:hover': {
            background: darkColors.alpha.black[5],
          },
          '&.Mui-selected.MuiButtonBase-root:hover': {
            background: darkColors.primary.main,
          },
        },
      },
    },
    MuiButton: {
      defaultProps: {
        disableRipple: true,
      },
      styleOverrides: {
        root: {
          fontWeight: 'bold',
          textTransform: 'none',
          paddingLeft: 16,
          paddingRight: 16,
          transition: 'all 0.3s ease-in-out',

          '.MuiSvgIcon-root': {
            transition: 'all .2s',
          },

          ':active': {
            transform: 'scale(0.98)',
          },
        },
        endIcon: {
          marginRight: -8,
        },
        containedSecondary: {
          backgroundColor: darkColors.secondary.main,
          color: darkColors.alpha.white[100],
          border: '1px solid ' + darkColors.alpha.black[30],
        },
        outlinedSecondary: {
          backgroundColor: darkColors.alpha.white[100],

          '&:hover, &.MuiSelected': {
            backgroundColor: darkColors.alpha.black[5],
            color: darkColors.alpha.black[100],
          },
        },
        sizeSmall: {
          padding: '6px 16px',
          lineHeight: 1.5,
        },
        sizeMedium: {
          padding: '8px 20px',
        },
        sizeLarge: {
          padding: '11px 24px',
        },
        textSizeSmall: {
          padding: '7px 12px',
        },
        textSizeMedium: {
          padding: '9px 16px',
        },
        textSizeLarge: {
          padding: '12px 16px',
        },
      },
    },
    MuiButtonBase: {
      defaultProps: {
        disableRipple: false,
      },
      styleOverrides: {
        root: {
          borderRadius: 6,
        },
      },
    },
    MuiToggleButton: {
      defaultProps: {
        disableRipple: true,
      },
      styleOverrides: {
        root: {
          color: darkColors.primary.main,
          background: darkColors.alpha.white[100],
          transition: 'all .2s',

          '&:hover, &.Mui-selected, &.Mui-selected:hover': {
            color: darkThemeColors.trueWhite,
            background: darkColors.primary.main,
          },
        },
      },
    },
    MuiIconButton: {
      styleOverrides: {
        root: {
          borderRadius: 8,
          padding: 8,

          '& .MuiTouchRipple-root': {
            borderRadius: 8,
          },

          ':active': {
            transform: 'scale(0.98)',
          },
        },
        sizeSmall: {
          padding: 4,
        },
      },
    },
    MuiListItemText: {
      styleOverrides: {
        root: {
          margin: 0,
        },
      },
    },
    MuiListItemButton: {
      styleOverrides: {
        root: {
          '& .MuiTouchRipple-root': {
            opacity: 0.3,
          },
        },
      },
    },
    MuiDivider: {
      styleOverrides: {
        root: {
          background: darkColors.alpha.black[10],
          border: 0,
          height: 1,
        },
        vertical: {
          height: 'auto',
          width: 1,

          '&.MuiDivider-flexItem.MuiDivider-fullWidth': {
            height: 'auto',
          },
          '&.MuiDivider-absolute.MuiDivider-fullWidth': {
            height: '100%',
          },
        },
        withChildren: {
          '&:before, &:after': {
            border: 0,
          },
        },
        wrapper: {
          background: darkColors.alpha.white[100],
          fontWeight: 'bold',
          height: 24,
          lineHeight: '24px',
          marginTop: -12,
          color: 'inherit',
          textTransform: 'uppercase',
        },
      },
    },
    MuiPaper: {
      styleOverrides: {
        root: {
          backgroundImage: 'none',
          padding: 0,
        },
        elevation0: {
          boxShadow: 'none',
        },
        elevation: {
          boxShadow: darkColors.shadows.card,
        },
        elevation2: {
          boxShadow: darkColors.shadows.cardSm,
        },
        elevation20: {
          boxShadow: darkColors.shadows.cardMd,
        },
        elevation24: {
          boxShadow: darkColors.shadows.cardLg,
        },
        outlined: {
          boxShadow: darkColors.shadows.card,
        },
      },
    },
    MuiLink: {
      defaultProps: {
        underline: 'hover',
      },
    },
    MuiLinearProgress: {
      styleOverrides: {
        root: {
          borderRadius: 6,
          height: 6,
        },
      },
    },
    MuiSlider: {
      styleOverrides: {
        root: {
          '& .MuiSlider-valueLabelCircle, .MuiSlider-valueLabelLabel': {
            transform: 'none',
          },
          '& .MuiSlider-valueLabel': {
            borderRadius: 6,
            background: darkColors.alpha.black[100],
            color: darkColors.alpha.white[100],
          },
        },
      },
    },
    MuiList: {
      styleOverrides: {
        root: {
          padding: 0,

          '& .MuiListItem-button': {
            transition: 'all .2s',

            '& > .MuiSvgIcon-root': {
              minWidth: 34,
            },

            '& .MuiTouchRipple-root': {
              opacity: 0.2,
            },
          },
          '& .MuiListItem-root.MuiButtonBase-root.Mui-selected': {
            backgroundColor: darkColors.alpha.black[10],
          },
          '& .MuiMenuItem-root.MuiButtonBase-root:active': {
            backgroundColor: alpha(darkColors.primary.lighter, 0.2),
          },
          '& .MuiMenuItem-root.MuiButtonBase-root .MuiTouchRipple-root': {
            opacity: 0.2,
          },
        },
        padding: {
          padding: '12px',

          '& .MuiListItem-button': {
            borderRadius: 6,
            margin: '1px 0',
          },
        },
      },
    },
    MuiTabs: {
      styleOverrides: {
        root: {
          height: 38,
          minHeight: 38,
          overflow: 'visible',
        },
        indicator: {
          height: 38,
          minHeight: 38,
          borderRadius: 6,
          border: '1px solid ' + darkColors.primary.dark,
          boxShadow: 'none',
        },
        scrollableX: {
          overflow: 'visible !important',
        },
      },
    },
    MuiTab: {
      styleOverrides: {
        root: {
          padding: 0,
          height: 38,
          minHeight: 38,
          borderRadius: 6,
          transition: 'color .2s',
          textTransform: 'capitalize',

          '&.MuiButtonBase-root': {
            minWidth: 'auto',
            paddingLeft: 20,
            paddingRight: 20,
            marginRight: 4,
          },
          '&.Mui-selected, &.Mui-selected:hover': {
            color: darkColors.alpha.trueWhite[100],
            zIndex: 5,
          },
          '&:hover': {
            color: darkColors.alpha.trueWhite[70],
          },
        },
      },
    },
    MuiMenu: {
      styleOverrides: {
        paper: {
          padding: 12,
        },
        list: {
          padding: 8,

          '& .MuiMenuItem-root.MuiButtonBase-root': {
            fontSize: 14,
            marginTop: 1,
            marginBottom: 1,
            transition: 'all .2s',
            color: darkColors.alpha.black[70],

            '& .MuiTouchRipple-root': {
              opacity: 0.2,
            },

            '&:hover, &:active, &.active, &.Mui-selected': {
              color: darkColors.alpha.black[100],
              background: alpha(darkColors.primary.lighter, 0.2),
            },
          },
        },
      },
    },
    MuiMenuItem: {
      styleOverrides: {
        root: {
          background: 'transparent',
          transition: 'all .2s',
          padding: '6px 10px',
          minHeight: 'auto',

          '&:hover, &:active, &.active, &.Mui-selected': {
            color: darkColors.alpha.black[100],
            background: alpha(darkColors.primary.lighter, 0.2),
          },
          '&.Mui-selected:hover': {
            background: alpha(darkColors.primary.lighter, 0.2),
          },
        },
      },
    },
    MuiListItem: {
      styleOverrides: {
        root: {
          '&.MuiButtonBase-root': {
            color: darkColors.secondary.main,

            '&:hover, &:active, &.active, &.Mui-selected': {
              color: darkColors.alpha.black[100],
              background: alpha(darkColors.primary.lighter, 0.2),
            },
          },
        },
      },
    },
    MuiAutocomplete: {
      styleOverrides: {
        tag: {
          margin: 1,
        },
        root: {
          '.MuiAutocomplete-inputRoot.MuiOutlinedInput-root .MuiAutocomplete-endAdornment': {
            right: 14,
          },
        },
        clearIndicator: {
          background: alpha(darkColors.error.lighter, 0.2),
          color: darkColors.error.main,
          marginRight: 8,

          '&:hover': {
            background: alpha(darkColors.error.lighter, 0.3),
          },
        },
        popupIndicator: {
          color: darkColors.alpha.black[70],

          '&:hover': {
            background: alpha(darkColors.primary.lighter, 0.2),
          },
        },
      },
    },
    MuiTablePagination: {
      styleOverrides: {
        toolbar: {
          '& .MuiIconButton-root': {
            padding: 8,
          },
        },
        select: {
          '&:focus': {
            backgroundColor: 'transparent',
          },
        },
      },
    },
    MuiToolbar: {
      styleOverrides: {
        root: {
          minHeight: '0 !important',
          padding: '0 !important',
        },
      },
    },
    MuiTableRow: {
      styleOverrides: {
        head: {
          background: darkColors.alpha.black[5],
        },
        root: {
          transition: 'background-color .2s',

          '&.MuiTableRow-hover:hover': {
            backgroundColor: alpha(darkColors.alpha.black[5], 0.05),
          },
        },
      },
    },
    MuiTableCell: {
      styleOverrides: {
        root: {
          borderBottomColor: darkColors.alpha.black[10],
          fontSize: 14,
        },
        head: {
          textTransform: 'uppercase',
          fontSize: 13,
          fontWeight: 'bold',
          color: darkColors.alpha.black[70],
        },
      },
    },
    MuiAlert: {
      styleOverrides: {
        message: {
          lineHeight: 1.5,
          fontSize: 14,
        },
        standardInfo: {
          color: darkColors.info.main,
        },
        action: {
          color: darkColors.alpha.black[70],
        },
      },
    },
    MuiTimelineDot: {
      styleOverrides: {
        root: {
          margin: 0,
          zIndex: 5,
          position: 'absolute',
          top: '50%',
          marginTop: -6,
          left: -6,
        },
        outlined: {
          backgroundColor: darkColors.alpha.white[100],
          boxShadow: '0 0 0 6px ' + darkColors.alpha.white[100],
        },
        outlinedPrimary: {
          backgroundColor: darkColors.alpha.white[100],
          boxShadow: '0 0 0 6px ' + darkColors.alpha.white[100],
        },
      },
    },
    MuiTimelineConnector: {
      styleOverrides: {
        root: {
          position: 'absolute',
          height: '100%',
          top: 0,
          borderRadius: 50,
          backgroundColor: darkColors.alpha.black[10],
        },
      },
    },
    MuiTimelineItem: {
      styleOverrides: {
        root: {
          minHeight: 0,
          padding: '8px 0',

          '&:before': {
            display: 'none',
          },
        },
        missingOppositeContent: {
          '&:before': {
            display: 'none',
          },
        },
      },
    },
    MuiTooltip: {
      styleOverrides: {
        tooltip: {
          backgroundColor: alpha(darkColors.alpha.black['100'], 0.95),
          padding: '5px 10px',
          fontSize: 13,
        },
        arrow: {
          color: alpha(darkColors.alpha.black['100'], 0.95),
        },
      },
    },
    MuiSwitch: {
      styleOverrides: {
        root: {
          height: 33,
          overflow: 'visible',

          '& .MuiButtonBase-root': {
            position: 'absolute',
            padding: 6,
            transition: 'left 150ms cubic-bezier(0.4, 0, 0.2, 1) 0ms,transform 150ms cubic-bezier(0.4, 0, 0.2, 1) 0ms',
          },
          '& .MuiIconButton-root': {
            borderRadius: 100,
          },
          '& .MuiSwitch-switchBase.Mui-checked + .MuiSwitch-track': {
            opacity: 0.3,
          },
        },
        thumb: {
          border: '1px solid ' + darkColors.alpha.black[30],
          boxShadow: '0px 9px 14px ' + darkColors.alpha.black[10] + ', 0px 2px 2px ' + darkColors.alpha.black[10],
        },
        track: {
          backgroundColor: darkColors.alpha.black[5],
          border: '1px solid ' + darkColors.alpha.black[10],
          boxShadow: 'inset 0px 1px 1px ' + darkColors.alpha.black[10],
          opacity: 1,
        },
        colorPrimary: {
          '& .MuiSwitch-thumb': {
            backgroundColor: darkColors.alpha.white[100],
          },

          '&.Mui-checked .MuiSwitch-thumb': {
            backgroundColor: darkColors.primary.main,
          },
        },
      },
    },
    MuiStepper: {
      styleOverrides: {
        root: {
          paddingTop: 20,
          paddingBottom: 20,
          background: darkColors.alpha.black[5],
        },
      },
    },
    MuiStepIcon: {
      styleOverrides: {
        root: {
          '&.MuiStepIcon-completed': {
            color: darkColors.success.main,
          },
        },
      },
    },
    MuiTypography: {
      defaultProps: {
        variantMapping: {
          h1: 'h1',
          h2: 'h2',
          h3: 'div',
          h4: 'div',
          h5: 'div',
          h6: 'div',
          subtitle1: 'div',
          subtitle2: 'div',
          body1: 'div',
          body2: 'div',
        },
      },
      styleOverrides: {
        gutterBottom: {
          marginBottom: 4,
        },
        paragraph: {
          fontSize: 17,
          lineHeight: 1.7,
        },
      },
    },
  },
  shape: {
    borderRadius: 10,
  },
  typography: {
    fontFamily:
      '"Inter", -apple-system, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji"',
    h1: {
      fontWeight: 700,
      fontSize: 35,
    },
    h2: {
      fontWeight: 700,
      fontSize: 30,
    },
    h3: {
      fontWeight: 700,
      fontSize: 25,
      lineHeight: 1.4,
      color: darkColors.alpha.black[100],
    },
    h4: {
      fontWeight: 700,
      fontSize: 22,
    },
    h5: {
      fontWeight: 700,
      fontSize: 18,
    },
    h6: {
      fontSize: 16,
    },
    body1: {
      fontSize: 15,
    },
    body2: {
      fontSize: 14,
    },
    button: {
      fontWeight: 600,
    },
    caption: {
      fontSize: 13,
      textTransform: 'uppercase',
      color: darkColors.alpha.black[50],
    },
    subtitle1: {
      fontSize: 14,
      color: darkColors.alpha.black[70],
    },
    subtitle2: {
      fontWeight: 400,
      fontSize: 15,
      color: darkColors.alpha.black[50],
    },
    overline: {
      fontSize: 13,
      fontWeight: 700,
      textTransform: 'uppercase',
    },
  },
  shadows: [
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
    'none',
  ],
});
